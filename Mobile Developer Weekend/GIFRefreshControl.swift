...
/*
 How To use by A-Hamdy
 
 let myrefreshControl = GIFRefreshControl()
 
 myrefreshControl.animatedImage = GIFAnimatedImage(data: NSData(contentsOfURL: NSBundle.mainBundle().URLForResource("(imageName)", withExtension: "gif")!)!)
 myrefreshControl.contentMode = .ScaleAspectFill
 myrefreshControl.addTarget(self, action: "Myrefresh", forControlEvents: .ValueChanged)
